...
const Hero = ({ isHome, label1, label2, label3, label4, label5, label6, vdo = homeVdo }) => {
  return (
    <>
      <div className='h-[100vh] bg-cover bg-no-repeat flex justify-center items-center font-serif font-medium relative inset-0 bg-black overflow-hidden'>
        ...
        <div className='w-full absolute text-center text-white md:text-[52px] 2xl:text-6xl leading-[80px] z-20 animate-fade-in-up '>
          {label1 && <p className='2xl:mb-6 font-semibold'>{label1}</p>}
          {label4 && (
            <p className='mb-1'>
              {label4.normal} <span className='italic font-semibold'>{label4.italic}</span>
            </p>
          )}
          {label2 && <p className='2xl:mb-6 font-semibold' >{label2}</p>}
          {label3 && <p className='italic font-semibold'>{label3}</p>}
          {label5 && <p className='text-3xl w-[68%] mx-auto font-semibold'>{label5}</p>}
          {label6 && <p className='text-[28px] w-full text-center font-thin'>{label6}</p>}
